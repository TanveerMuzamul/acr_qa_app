...
from app.services.acr_legacy.dataset import Series, get_img, plane, to_float
...
def localizer_length_measure(s: Series, idx: int = 1) -> dict[str, Any]:
    img = get_img(s, idx)
    arr = img.arr
    d = img.dcm
...
def evaluate_geometric(localizer_s: Optional[Series], t1_s: Optional[Series]) -> dict[str, Any]:
    ...
    if localizer_s is not None and localizer_s.n >= 1:
        loc = localizer_length_measure(localizer_s, 1)
...
def save_geometric_debug_localizer(s: Series, idx: int, out_dir: str | Path) -> Optional[Path]:
    ...
    ax.set_title("localizer geometric")
    ax.set_axis_off()
