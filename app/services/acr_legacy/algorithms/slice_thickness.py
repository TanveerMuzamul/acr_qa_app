...
def insert_roi(arr: np.ndarray) -> tuple[np.ndarray, tuple[int, int, int, int]]:
    ...
    search = arr[sy0:sy1, sx0:sx1].astype(np.float32)

    row_dark = 1.0 - norm01(search).mean(axis=1)
    row_dark = smooth1d(row_dark, 11)

    y_mid = int(np.argmax(row_dark))
    band_h = max(14, int(0.10 * ph))

    ry0 = sy0 + max(0, y_mid - band_h // 2)
    ry1 = sy0 + min(search.shape[0], y_mid + band_h // 2)
...
def best_segment(mask: np.ndarray) -> Optional[tuple[int, int]]:
    idx = np.where(mask)[0]
    if idx.size == 0:
        return None
    ...
    segs.sort(key=lambda s: (s[1] - s[0] + 1), reverse=True)
    return segs[0]
...
def ramp_profile(roi_n: np.ndarray, half: str) -> dict[str, Any]:
    ...
    row_sig = smooth1d(part[:, x0:x1].mean(axis=1), 5)
    ...
    prof = smooth1d(prof, 7)

    base = float(np.percentile(prof[x0:x1], 10))
    peak = float(np.percentile(prof[x0:x1], 95))
    ...
    seg = best_segment(m)
    if seg is None:
        xm = int(np.argmax(prof[x0:x1])) + x0
        seg = (max(x0, xm - 3), min(x1 - 1, xm + 3))
