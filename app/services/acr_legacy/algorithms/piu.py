...
def small_roi_centers(shape: tuple[int, int], cx: float, cy: float, r_large: float, r_small: float) -> np.ndarray:
    h, w = shape
    yy, xx = np.ogrid[:h, :w]
    rr = max(0.0, r_large - r_small - 1.0)
...
def best_high_low(arr: np.ndarray, large: dict[str, Any], ps: Optional[list[float]]) -> dict[str, Any]:
    ...
    centers_mask = small_roi_centers(arr.shape, large["cx"], large["cy"], large["r_px"], r_small)
